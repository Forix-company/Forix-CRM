...
@section('page-inner')
    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
        <div>
            <h2 class="text-white pb-2 fw-bold">Modulo Contabilidad Cuentas por cobrar</h2>
            <h5 class="text-white op-7 mb-2">En este Modulo puede Visualizar el Total de Cuentas por cobrar</h5>
        </div>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="multi-filter-select" class="display table table-striped table-hover">
                                ...
                                <tbody>
                                    @foreach ($AccountsReceivable as $Bank)
                                        <tr>
                                            <td>{{ $Bank->id }}</td>
                                            <td>{{ $Bank->name_customer }}</td>
                                            <td>{{ $Bank->description }}</td>
                                            <td>{{ "$" . number_format($Bank->AmountReceivable, 2, '.', ',') }}</td>
    ...
    <div class="modal fade" id="create" tabindex="-1" role="dialog" data-backdrop="static">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                {!! Form::open(['route' => 'contabilidad.AccountsReceivable.store']) !!}
                <div class="modal-body text-center">
                    <div class="row">
                        <div class="col-sm-12 form-group">
                            <h3>AGREGAR INFORMACION DE LA CUENTA POR COBRAR</h3>
                        </div>
                        <div class="col-sm-4 form-group">
                            {!! Form::label('Seleccione el Cliente', 'Seleccione el Cliente') !!} <span style="color: red">*</span>
                            <select name="customer" class="form-control" required>
                                @foreach ($Cliente as $Clientes)
                                    <option value="" selected disabled>Seleccione una Opcion</option>
                                    <option value="{{ $Clientes->id }}">{{ $Clientes->name_customer }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-sm-4 form-group">
