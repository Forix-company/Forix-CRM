                                ...
                                <tr>
                                    <td class="content-cell"
                                        ...
                                        <p
                                            ...
                                            Puedes realizar tu pago en nuestra pasarela de pago</p>
                                        <p
                                            style="box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; position: relative; font-size: 16px; line-height: 1.5em; margin-top: 0; text-align: left;">
                                            Puedes consultar los productos adquiridos</p>
                                        <table class="action" align="center" width="100%" cellpadding="0"
                                            ...
                                            <tbody>
                                                <tr>
                                                    <td>{{ $Messaje['description'] }}</td>
                                                    <td>{{ $Messaje['precio'] }}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                        <center>
                                            <form method="post" action="{{ $Messaje['URL'] }}">
                                                <input name="referenceCode" type="hidden"
                                                    value="{{ $Messaje['referenceCode'] }}">
                                                <input name="description" type="hidden"
                                                    value="{{ $Messaje['description'] }}">
                                                <input name="amount" type="hidden" value="{{ $Messaje['amount'] }}">
                                                <input name="buyerEmail" type="hidden"
                                                    value="{{ $Messaje['buyerEmail'] }}">

                                                <input name="tax" type="hidden" value="{{ $Messaje['tax'] }}">
                                                <input name="taxReturnBase" type="hidden"
                                                    value="{{ $Messaje['taxReturnBase'] }}">
                                                <input name="currency" type="hidden"
                                                    value="{{ $Messaje['currency'] }}">
                                                <input name="merchantId" type="hidden"
                                                    value="{{ $Messaje['merchantId'] }}">
                                                <input name="accountId" type="hidden"
                                                    value="{{ $Messaje['accountId'] }}">
                                                <input name="signature" type="hidden"
                                                    value="{{ $Messaje['signature'] }}">
                                                <input name="test" type="hidden" value="{{ $Messaje['test'] }}">
                                                <input name="responseUrl" type="hidden"
                                                    value="{{ $Messaje['responseUrl'] }}">
                                                <input name="Submit"
                                                    style="display: inline-block;padding: 10px 20px;font-size: 16px;font-weight: bold;text-align: center;text-decoration: none;
                                                background-color: #007bff;color: white;border: none;border-radius: 4px;cursor: pointer;"
                                                    type="submit" value="Realizar Pago">
                                            </form>
                                        </center>
                                        <p
                                            style="box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; position: relative; font-size: 16px; line-height: 1.5em; margin-top: 0; text-align: left;">
                                            El presente correo es emitido de manera automática y no se encuentra
                                            habilitado para recibir respuestas o consultas.
                                            Recomendamos incluir esta cuenta a la lista de contactos evitando que llegue
                                            en “Correo no deseado”.
                                            Para modificar o cancelar la manera de recibir su Extracto Unificado,
                                            comunicarse con la línea de Servicio al Cliente para la actualización de
                                            datos correspondiente. </p>
                                    </td>
                                </tr>
                            </table>
                        </td>
                                ...
                                <tr>
                                    <td class="content-cell" align="center"
                                        ...
                                        <p
                                            ...
                                            © 2022 Software. All rights reserved.</p>
                                    </td>
                                </tr>
                            </table>
                        </td>
